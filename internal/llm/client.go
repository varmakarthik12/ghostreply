...
func (c *Client) chatOpenAI(ctx context.Context, model string, msgs []Message, contextSize int, params SamplingParams) (string, Stats, error) {
	...
	type openAIMessagePart struct {
		...
		InputVideo *openAIVideo    `json:"input_video,omitempty"`
		...
		if len(m.Images) > 0 || len(m.Audios) > 0 || len(m.Videos) > 0 {
			...
			for _, vid := range m.Videos {
				...
				if strings.HasPrefix(vid, "data:") {
					partsData := strings.SplitN(vid, ";base64,", 2)
					if len(partsData) == 2 {
						mime := strings.TrimPrefix(partsData[0], "data:")
						if strings.HasPrefix(mime, "video/") {
							format = strings.TrimPrefix(mime, "video/")
						}
						data = partsData[1]
					}
				}
